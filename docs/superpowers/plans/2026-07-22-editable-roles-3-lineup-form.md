# Editable Roles & Instruments — Plan 3: Lineup Form Implementation Plan

> **For agentic workers:** REQUIRED SUB-SKILL: Use superpowers:subagent-driven-development (recommended) or superpowers:executing-plans to implement this plan task-by-task. Steps use checkbox (`- [ ]`) syntax for tracking.

**Goal:** Make the lineup create/edit form (`LineupFormPage.jsx`) render its worship-leader role options and instrument slots from the global config instead of hardcoded arrays, while still correctly loading and editing lineups saved in the old fixed-field format.

**Architecture:** The form's state gains `instrumentAssignments: { [slotId]: memberId[] }` (replacing the old `instruments`/`soundEngineer` fields) and `activeExtraSlotIds: string[]` (which non-core slots currently have a visible row, replacing the old `instruments.extras` array). Existing lineups are converted into this shape on load via `normalizeLineupInstruments` (Plan 1). The "Instrumentalists" grid and "Additional Instruments" picker both loop over `useApp().instrumentSlots` instead of hardcoded JSX/arrays; the Team A role `<select>` loops over `useApp().worshipLeaderRoles`.

**Tech Stack:** React (Vite), Tailwind CSS, lucide-react icons, `@dnd-kit` (already used for song reordering, untouched by this plan). Plain JavaScript, no TypeScript.

## Global Constraints

- **Requires Plan 1 (foundation) to be merged first.** This plan consumes `instrumentSlots`, `worshipLeaderRoles` from `useApp()`, and `normalizeLineupInstruments` from `src/utils/normalizeLineupInstruments.js` — all added by Plan 1.
- No test framework exists in this repo. Verification is manual: `npm run dev` + exercise the form in the browser for both a brand-new lineup and an existing (pre-migration) lineup. Do not add a test framework.
- Plain JavaScript only — no TypeScript syntax.
- Do not touch the Songs section, DnD reordering, practice date/timing, theme, Bible verse, Next WL, or Notes sections of this file — none of that is in scope, and the design doc for this feature explicitly excludes `SONG_SECTIONS`.
- **Deliberate behavior change, called out here so it isn't mistaken for a bug:** previously, the "Additional Instruments" picker was a free array of rows, so the same instrument (e.g. "Violin") could technically be added as two separate rows. Under the new model each non-core instrument slot can only have one active row (its member list already supports selecting multiple people). This is an intentional simplification — do not try to preserve the old duplicate-row behavior.
- A lineup document with real production data can be loaded into this form at any time — do not write anything that could silently drop data. `normalizeLineupInstruments` already guarantees this; don't bypass it.

---

### Task 1: Refactor `LineupFormPage.jsx` to be config-driven

**Files:**
- Modify: `src/pages/LineupFormPage.jsx` (complete rewrite of this file's contents — the changes touch imports, module-level constants, form state shape, several handlers, and multiple JSX sections closely enough that a full-file replacement is safer than a series of partial edits)

**Interfaces:**
- Consumes: `useApp()` → `instrumentSlots`, `worshipLeaderRoles` (Plan 1); `normalizeLineupInstruments` from `../utils/normalizeLineupInstruments` (Plan 1).
- Produces: no exports consumed elsewhere except `EXTRA_INSTRUMENTS`, which this task removes — confirmed unused anywhere else in the codebase (only this file imported it, and only for its own local use).

- [ ] **Step 1: Replace the entire file**

Overwrite `src/pages/LineupFormPage.jsx` with:
...
Run: `npm run lint` — expected: no new errors or warnings for `src/pages/LineupFormPage.jsx` (in particular, no "unused variable" warnings for anything removed — `EXTRA_INSTRUMENTS`, `TEAM_A_ROLES`, `INSTRUMENT_ROLES`, `AudioLines`/`Bell`/`Repeat2` icon imports are all gone now that nothing references them).

- [ ] **Step 3: Manual verification — new lineup**

Run: `npm run dev`, sign in as an admin, go to "New Lineup". Confirm:
1. The Instrumentalists grid shows Keyboard 1, Keyboard 2, Bass, Lead Guitar, Acstc Guitar, Drums (in that order, 2-per-row) — same as before.
2. Sound Engineer still renders as its own separate card below.
3. Checking "This is a Team A Sunday" switches the worship-leader role dropdown to show Opening/Welcome, Praise, Worship, Lord's Table, Opening, Other.
4. Clicking "Add Instrument" adds a row defaulting to the first optional instrument (Violin), with a dropdown to change it and a member picker below.
5. Selecting members in a core slot marks them as "(assigned)"/disabled in other slot dropdowns, same as before.
6. Saving a full lineup succeeds and redirects to its detail page (verified further in Plan 4 once that lands, but confirm no console errors on save here).

- [ ] **Step 4: Manual verification — editing an existing (pre-migration) lineup**

Still in the dev server, open one of the seeded lineups (e.g. `2026-01-04`) for editing (`/lineup/lineup-2026-01-04/edit`). Confirm:
1. All previously-assigned members show up correctly in Keyboard 1/2, Bass, Lead Guitar, Acoustic Guitar, Drums, and Sound Engineer — this is the `normalizeLineupInstruments` compatibility path working on real (well, seeded) legacy-format data.
2. Making an edit and saving succeeds with no console errors.
3. Re-opening the same lineup for edit afterward still shows the edit correctly (confirms the new `instrumentAssignments` format round-trips through Firestore correctly).

- [ ] **Step 5: Commit**

```bash
git add src/pages/LineupFormPage.jsx
git commit -m "feat: make lineup form's roles and instrument slots config-driven"
```
