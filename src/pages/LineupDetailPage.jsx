        ...
        <div>
          ...
          {lineup.practiceDate && (
            <div className="mt-2 inline-flex items-center gap-1.5 bg-teal-50 border border-teal-200 text-teal-700 rounded-lg px-3 py-1.5 text-xs font-semibold">
              ...
              Practice: {shortDate(lineup.practiceDate)}{getPracticeTimingLabel(lineup.practiceTiming) ? `, ${getPracticeTimingLabel(lineup.practiceTiming)}` : ''}
            </div>
          )}
        </div>
          ...
          <>
            ...
            <div>
              ...
              <div className="space-y-3">
                {(() => {
                  // Pair Opening + Welcome side-by-side; everything else renders normally
                  ...
                  // Insert paired Opening + Welcome row first (at the position of whichever comes first)
                  if (paired) {
                  ...
                  songGroups.forEach((group, gi) => {
                    if (!pairedIndices.has(gi)) {
                      rows.push({ type: 'single', insertAt: gi, group });
                    }
                  });

                  // Sort by original index so order is preserved
